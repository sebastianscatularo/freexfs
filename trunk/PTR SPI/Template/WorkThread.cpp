...
LRESULT CWorkThread::OnSPGetInfo(WPARAM wParam, LPARAM lParam)
{
	...
	{		
		...
		HRESULT h = WFMAllocateBuffer(sizeof(WFSPTRSTATUS),WFS_MEM_ZEROINIT,(void **)&lpStatus);
		pServiceBasic->GetDevStatus(lpStatus);
	}
	else if(pServiceBasic->m_dwCategory == WFS_INF_PTR_CAPABILITIES)
	{
			
		HRESULT h = WFMAllocateBuffer(sizeof(WFSPTRCAPS),WFS_MEM_ZEROINIT,(void **)&lpCaps);
	...
	else if(pServiceBasic->m_dwCategory == WFS_INF_PTR_FORM_LIST)
	{
		...
		if(cbNumbers > 0)
		{
			h = WFMAllocateBuffer(cbNumbers * 200,WFS_MEM_ZEROINIT,(void **)&lpForms);
			nLength = 0;
	...
	{
		HRESULT h = WFMAllocateBuffer(sizeof(WFSFRMHEADER),WFS_MEM_ZEROINIT,(void **)&lpFormHeader);
		lpFormHeader->lpszFormName = (char *)pServiceBasic->m_lpQueryDetails;
		CString strFileName = pServiceBasic->GetFormFilePath(
			m_strLogicalName,lpFormHeader->lpszFormName);
		pServiceBasic->QueryForm(lpFormHeader, strFileName);
	}
	else if(pServiceBasic->m_dwCategory == WFS_INF_PTR_QUERY_FIELD)
	{
		...
		HRESULT h = WFMAllocateBuffer(sizeof(WFSFRMFIELD),WFS_MEM_ZEROINIT,(void **)&lpFormField);
		CString strFileName = pServiceBasic->GetFormFilePath(
			m_strLogicalName,pWFSPTRQUERYFIELD->lpszFormName);
	...
	else if(pServiceBasic->m_dwCategory == WFS_INF_PTR_QUERY_FIELD) pResult->lpBuffer = lpFormField;
